...
export const PendingSection = ({pendingRequest}:{pendingRequest:AssetRequestType[]}) => {
  ...
  return (
      <div className="space-y-5">
          ...
          <div className={`grid grid-cols-1 md:grid-cols-2  gap-2 ${pendingRequest.length>2 && 'lg:grid-cols-3 xl:grid-cols-4'}`}>
             {
              pendingRequest?.map(request=><DisplayPendingRequest key={request._id} request={request} />)
          }  
          </div>
