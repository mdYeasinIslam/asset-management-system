...
export const RequestAssets = () => {
    ...
    try {
      ...
      if (assetName) {
        const response = await axiosSecure.get(
          `/assets?name=${assetName}&availabilty=${status}&public=public`
        );
        ...
        setValue(status);
        const response = await axiosSecure.get(
          `/assets?name=${assetName}&availabilty=${status}`
        );
  ...
  return (
    <section className="container mx-auto space-y-10 py-10 dark:text-white">
          ...
          <Select>
            ...
            <SelectContent>
              <div className="flex flex-col gap-2">
                <Button
                  onClick={() => filterAssets("", "available")}
                  className={`bg-white text-black hover:bg-primary hover:text-white w-full ${
                    value == "available" ? "bg-primary text-white" : ""
                  }`}
                >
                  Available
                </Button>
                <Button
                  onClick={() => filterAssets("", "outOfStock")}
                  className={`bg-white text-black hover:bg-primary hover:text-white w-full ${
                    value == "outOfStock" ? "bg-primary text-white" : ""
                  }`}
                >
                  Out of stock
                </Button>
      ...
      {error?.length ? (
        <div className="text-3xl font-semibold ">{error}</div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 px-2 xl:px-0">
          {data?.length ? (
            <>
              {data?.map((assets: AssetType) => (
                <DisplayAllAssets key={assets._id} assets={assets} />
              ))}
            </>
          ) : (
            <>
              {assetsData?.map((assets: AssetType) => (
                <DisplayAllAssets key={assets._id} assets={assets} />
              ))}
            </>
          )}
